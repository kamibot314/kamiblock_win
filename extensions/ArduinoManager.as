package extensions
{
	...
	import extension2.ArudinoUploader;
	  ...
	  *var = v;
	  lineParsed = true;
		...
		private function parseCodeBlocks(blks:Object):Boolean{
			...
			for(var i:int;i<blks.length;i++){
				...
				if(b[0].indexOf("runArduino")>-1){
					...
					var objs:Array = Main.app.extensionManager.specForCmd(blks[0]);
					var n:String = blks[0];
					var ext:ScratchExtension = Main.app.extensionManager.extensionByName(n.split(".")[0]);
					if(ext!=null){
						if(srcDocuments.indexOf(ext.srcPath)==-1){
							srcDocuments.push(ext.srcPath);
						}
		...
		public function jsonToCpp(code:String):String{
			...
			if("scripts" in objs){
				for(var j:uint=0;j<objs.scripts.length;j++){
					var scr:Object = objs.scripts[j][2];
		...
		private function prepareProjectDir(ccode:String):void{
			...
			var srcdir:File = File.applicationDirectory.resolvePath("ext/"+_extSrcPath+"/src");
...
//			srcdir.copyTo(workdir,true); 
			for each(var path:String in srcDocuments){
				trace("srcdir = " + srcdir.nativePath);
...
//			}
		...
		public function get projectDocumentName():String{
			...
			var pName:String = Main.app.projectName().split(" ").join("");
		...
		public function buildAll(ccode:String):String
		{
			...
			var file:File = new File(projectPath+"/"+projectDocumentName+".ino");
//			arduinoInstallPath2.url = arduinoInstallPath;
//			arduinoInstallPath2 = File.applicationDirectory.resolvePath("Arduino");
			trace(arduinoInstallPath.nativePath);
			trace(file.nativePath);
			
			var boardCodeName:String = BoardManager0.getInstance().boardData.codename;
			var mcu:String = BoardManager0.getInstance().boardData.mcu.codename;
			
			
			var currentPort:String = SerialPortManager.getInstance().port;
			
			function onUploadEnd(e:Event):void {
				trace("upload ended");
				isUploading = false;
				currentPort = SerialPortManager.getInstance().port;
				
				SerialManager.sharedManager().open(SerialPortManager.getInstance().port);
			}
			
			function onComplieComplete(e:Event):void {
				trace("onComplieComplete");
				...
				ArudinoUploader.getInstance().removeEventListener(Event.COMPLETE,onUploadEnd);
				ArudinoUploader.getInstance().addEventListener(Event.COMPLETE,onUploadEnd);
				trace("uploads at " + currentPort);
				ArudinoUploader.getInstance().upload(
					arduinoInstallPath,
					BoardManager0.getInstance().boardData.mcu,
					currentPort,
					ArudinoCompiler.getInstance().hexFile);
			}
			
			ArudinoCompiler.getInstance().removeEventListener(Event.COMPLETE,onComplieComplete);
			ArudinoCompiler.getInstance().addEventListener(Event.COMPLETE,onComplieComplete)
			
			ArudinoCompiler.getInstance().complie(arduinoInstallPath,file,boardCodeName,mcu);
			trace("complie start");
		...
		public function openArduinoIDE(ccode:String):String{
			...
			if(ApplicationManager.sharedManager().system==ApplicationManager.WINDOWS){
				file = arduinoInstallPath.resolvePath("arduino.exe");;
			}else{
				file = arduinoInstallPath.resolvePath("../../MacOS/JavaApplicationStub");
				if(!file.exists){
					file.url = arduinoInstallPath.resolvePath("../MacOS/Arduino").url; 
				}
			}
		...
		private function buildArduinoLib(buildDir:File):void{
			...
			listArduinoLib(file)
			// enum arduino libs
			file.url = new File(arduinoInstallPath+avrPath+"/ext").url;
			if(file.exists){
				arduinoLibPath = avrPath+"/ext";
			}else{
				arduinoLibPath = "/ext";
			}
			...
			listArduinoLib(file)
			file.url = new File(arduinoInstallPath+"/ext/Servo").url; // servo still in root library
			listArduinoLib(file)
		...
		*/
		...
		public function get arduinoInstallPath():File{
			return File.applicationDirectory.resolvePath("Arduino");
			
//			if(arduinoPath.length>0){
//				var ttf:File = new File();
//				trace("arduinoPath = " + arduinoPath);
...
//			return arduinoPath;
		...
		private function compileCpp(cpp:String,dir:File):void{
			...
			if(_currentDevice=="uno"){
				cmd = " -c -g -Os -w -fno-exceptions -ffunction-sections -fdata-sections -MMD -mmcu=atmega328p -DF_CPU=16000000L -DARDUINO=156 -DARDUINO_AVR_UNO -DARDUINO_ARCH_AVR -I"+path+avrPath+"/cores/arduino -I"+path+avrPath+"/variants/standard -I"+path+"/ext/Servo/src -I"+path+"/ext/Servo -I"+path+arduinoLibPath+"/Wire -I"+path+arduinoLibPath+"/Wire/utility -I"+path+arduinoLibPath+"/SoftwareSerial"
			}else if(_currentDevice=="leonardo"){
				cmd = " -c -g -Os -w -fno-exceptions -ffunction-sections -fdata-sections -MMD -mmcu=atmega32u4 -DF_CPU=16000000L -DARDUINO=156 -DARDUINO_AVR_LEONARDO -DARDUINO_ARCH_AVR -DUSB_VID=0x2a03 -DUSB_PID=0x8036 -DUSB_MANUFACTURER= -DUSB_PRODUCT=\"Arduino Leonardo\" -I"+path+avrPath+"/cores/arduino -I"+path+avrPath+"/variants/leonardo -I"+path+"/ext/Servo/src -I"+path+"/ext/Servo -I"+path+arduinoLibPath+"/Wire -I"+path+arduinoLibPath+"/Wire/utility -I"+path+arduinoLibPath+"/SoftwareSerial"
			}else if(_currentDevice=="mega1280"){
				cmd = " -c -g -Os -w -fno-exceptions -ffunction-sections -fdata-sections -mmcu=atmega1280 -DF_CPU=16000000L -MMD -DUSB_VID=null -DUSB_PID=null -DARDUINO=1062 -I"+path+avrPath+"/cores/arduino -I"+path+avrPath+"/variants/mega -I"+path+"/ext/Servo/src -I"+path+"/ext/Servo -I"+path+arduinoLibPath+"/Wire -I"+path+arduinoLibPath+"/Wire/utility -I"+path+arduinoLibPath+"/SoftwareSerial"
			}else if(_currentDevice=="mega2560"){
				cmd = " -c -g -Os -w -fno-exceptions -ffunction-sections -fdata-sections -mmcu=atmega2560 -DF_CPU=16000000L -MMD -DUSB_VID=null -DUSB_PID=null -DARDUINO=1062 -I"+path+avrPath+"/cores/arduino -I"+path+avrPath+"/variants/mega -I"+path+"/ext/Servo/src -I"+path+"/ext/Servo -I"+path+arduinoLibPath+"/Wire -I"+path+arduinoLibPath+"/Wire/utility -I"+path+arduinoLibPath+"/SoftwareSerial"
			}else if(_currentDevice=="nano328"){
				cmd = " -c -g -Os -w -fno-exceptions -ffunction-sections -fdata-sections -mmcu=atmega328p -DF_CPU=16000000L -MMD -DUSB_VID=null -DUSB_PID=null -DARDUINO=1062 -I"+path+avrPath+"/cores/arduino -I"+path+avrPath+"/variants/eightanaloginputs -I"+path+"/ext/Servo/src -I"+path+"/ext/Servo -I"+path+arduinoLibPath+"/Wire -I"+path+arduinoLibPath+"/Wire/utility -I"+path+arduinoLibPath+"/SoftwareSerial"
			}else if(_currentDevice=="nano168"){
				cmd = " -c -g -Os -w -fno-exceptions -ffunction-sections -fdata-sections -mmcu=atmega168 -DF_CPU=16000000L -MMD -DUSB_VID=null -DUSB_PID=null -DARDUINO=1062 -I"+path+avrPath+"/cores/arduino -I"+path+avrPath+"/variants/eightanaloginputs -I"+path+"/ext/Servo/src -I"+path+"/ext/Servo -I"+path+arduinoLibPath+"/Wire -I"+path+arduinoLibPath+"/Wire/utility -I"+path+arduinoLibPath+"/SoftwareSerial"
			}
		...
		private function compileC(cpp:String, dir:File):void{
			...
			var path:String = arduinoInstallPath;
			path=path.split("file:///").join("");//.split("/").join("/");
			//			var cmd:String = " -c -g -Os -w -ffunction-sections -fdata-sections -MMD -mmcu=atmega32u4 -DF_CPU=16000000L -DARDUINO=156 -DARDUINO_AVR_LEONARDO -DARDUINO_ARCH_AVR -DUSB_VID=0x2341 -DUSB_PID=0x8036 -DUSB_MANUFACTURER= -DUSB_PRODUCT=\"Arduino Leonardo\" -I"+path+avrPath+"/cores/arduino -I"+path+avrPath+"/variants/leonardo -I"+path+arduinoLibPath+"/Wire -I"+path+"/ext/Servo/src -I"+path+"/ext/Servo -I"+path+arduinoLibPath+"/SoftwareSerial -I"+path+arduinoLibPath+"/Wire/utility"
			//			if(boardType!="leonardo")
			//				cmd = " -c -g -Os -w -ffunction-sections -fdata-sections -MMD -mmcu=atmega328p -DF_CPU=16000000L -DARDUINO=156 -DARDUINO_AVR_UNO -DARDUINO_ARCH_AVR -I"+path+avrPath+"/cores/arduino -I"+path+avrPath+"/variants/standard -I"+path+arduinoLibPath+"/Wire -I"+path+"/ext/Servo/src -I"+path+"/ext/Servo -I"+path+arduinoLibPath+"/SoftwareSerial -I"+path+arduinoLibPath+"/Wire/utility"
			var cmd:String = "";
			if(_currentDevice=="uno"){
				cmd = " -c -g -Os -w -ffunction-sections -fdata-sections -MMD -mmcu=atmega328p -DF_CPU=16000000L -DARDUINO=156 -DARDUINO_AVR_UNO -DARDUINO_ARCH_AVR -I"+path+avrPath+"/cores/arduino -I"+path+avrPath+"/variants/standard -I"+path+"/ext/Servo/src -I"+path+"/ext/Servo -I"+path+arduinoLibPath+"/Wire -I"+path+arduinoLibPath+"/Wire/utility -I"+path+arduinoLibPath+"/SoftwareSerial"
			}else if(_currentDevice=="leonardo"){
				...
				cmd = " -c -g -Os -w -ffunction-sections -fdata-sections -mmcu=atmega168 -DF_CPU=16000000L -MMD -DUSB_VID=null -DUSB_PID=null -DARDUINO=1062 -I"+path+avrPath+"/cores/arduino -I"+path+avrPath+"/variants/eightanaloginputs -I"+path+"/ext/Servo/src -I"+path+"/ext/Servo -I"+path+arduinoLibPath+"/Wire -I"+path+arduinoLibPath+"/Wire/utility -I"+path+arduinoLibPath+"/SoftwareSerial"
			}
