...
public class MediaLibraryItem extends Sprite {
	...
	private function setImageThumbnail(md5:String, done:Function, spriteMD5:String = null):void {
		...
		if (fileType(md5) == 'svg') loaders.push(Main.app.server.getAsset(md5, gotSVGData));
		else loaders.push(Main.app.server.getThumbnail(md5, thumbnailWidth, thumbnailHeight, setThumbnail));
	}
	...
	private function setSpriteThumbnail(done:Function):void {
		...
		else loaders.push(Main.app.server.getAsset(spriteMD5, gotJSONData));
	}
	...
	private function downloadAndPlay():void {
		...
		Main.app.server.getAsset(dbObj.md5, gotSoundData);
	}
