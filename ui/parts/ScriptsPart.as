...
public class ScriptsPart extends UIPart {
	...
	private var uploadBt:Button = new Button(Translator.map("Upload to KamiBot"));
...
//		arduinoTextPane.type = TextFieldType.INPUT;
	...
	private function onCompileArduino(evt:MouseEvent):void{
		if(SerialManager.sharedManager().isConnected){
			if(ArduinoManager.sharedManager().isUploading==false){
				...
				if(parseArduinoCodeByStage()){
					messageTextPane.append(ArduinoManager.sharedManager().arduinoInstallPath.nativePath);
					messageTextPane.append(ArduinoManager.sharedManager().buildAll(arduinoTextPane.textField.text));
				}
			}
		}else{
			if (!ArduinoManager.sharedManager().isUploading) {
	...
	public function parseArduinoCodeByStage(arg:String=""):Boolean{
		var json:String = util.JSON.stringify(app.stagePane);
		var formatCode:String = ArduinoManager.sharedManager().jsonToCpp(json);
...
//		formatKeyword(arduinoTextPane.textField,"Arduino",fontGreen,0,0);
	...
	public function updateTranslation():void{
		...
		uploadBt.setLabel(Translator.map("Upload to KamiBot"));
		openBt.setLabel(Translator.map("Edit with Arduino Editor"));
