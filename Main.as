package {
	...
	public class Main extends Sprite {
		...
		private var showingConnectMenu:Boolean = false;
		...
		private static const _currentVer:String = "17.03.24";
...
//			var isFilesAvailable:Boolean = File.applicationStorageDirectory.resolvePath("mBlock").exists;
//			if(!isFilesAvailable){
...
//			function connectMenu():void {
//				showingConnectMenu = false;
//			}
//			
//			if (!showingConnectMenu) {
//				showingConnectMenu = true;
//				SerialPortManager.getInstance().listPort(connectMenu);
//			}
			...
			for(var i:uint=0;i<arr.length;i++){
				m.addItem(arr[i], "serial_"+arr[i], enabled, SerialPortManager.getInstance().port.indexOf(arr[i])>-1&&SerialManager.sharedManager().isConnected);
			}
			m.addLine();
			m.addItem("Program", "", false, false);
			if(BoardManager0.getInstance().board.toLocaleLowerCase().indexOf("kamibot") !== -1){
				m.addItem(Translator.map("Install KamiBot Program"), 'scratch_firmware', SerialManager.sharedManager().isConnected, false);
			}
			m.addItem('Install KamiBot Driver', 'driver', true, false);
			
			var connectX:uint = b.x;
//			trace("connectX = " + connectX);
			
			m.showOnStage(stage, b.x, topBarPart.bottom() - 1);
			
			
			SerialPortManager.getInstance().listPort(function(x:uint){
				...
				var arr:Array = SerialManager.sharedManager().list;
				for(var i:uint=0;i<arr.length;i++){
					var s:String = Translator.map(SerialPortManager.getInstance().getVendor(arr[i]));
					if (s !== null) {
						s = " (" + s + ")";
					} else {
						s = "";
					}
					m.addItem(arr[i] + s, "serial_"+arr[i], enabled, SerialPortManager.getInstance().port.indexOf(arr[i])>-1&&SerialManager.sharedManager().isConnected);
				}
				m.addLine();
				m.addItem("Program", "", false, false);
				if(BoardManager0.getInstance().board.toLocaleLowerCase().indexOf("kamibot") !== -1){
...
//				trace("x = " + x);
				m.showOnStage(stage, x, topBarPart.bottom() - 1);
				Menu.removeMenusFrom(stage);//remove old menu
			},connectX);
		}
