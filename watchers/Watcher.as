...
public class Watcher extends Sprite implements DragClient {
	...
	private function updateLabel():void {
		...
		if (cmd == 'getVar:') {
			...
			setLabel((target.isStage ? '' : target.objName + ': ') + Translator.map(specForCmd()));
		}
	}
	...
	private function specForCmd():String {
		...
		if(i > -1) {
			var spec:Array = Main.app.extensionManager.specForCmd(cmd);
	...
	private function getValue(runtime:ScratchRuntime):* {
		...
		if(cmd.indexOf('.') > -1) {
			...
			if(spec) {
				...
				return Main.app.interp.evalCmd(block);
			}
		}
	...
	public function menu(evt:MouseEvent):Menu {
		function handleMenu(item:int):void {
			...
			if (item == 10) {
				visible = false;
				Main.app.updatePalette(false);
			}
		}
		if (!Main.app.editMode) return null;
...
//			m.addItem("text", 4);
	...
	private function mouseDown(evt:MouseEvent):void {
		...
		if (p.y > 20) Main(root).gh.setDragClient(this, evt);
	}
